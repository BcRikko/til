...
[background-image](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Properties/background-image)は適切なroleやsourceを指定しない限り、支援技術には認識されない。そのため、主に修飾目的として使われる。そのため、`dark-light(<image>, <image>)`は用途を限定して使う必要がある。
